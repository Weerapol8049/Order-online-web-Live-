      ...
      data.map((row) => {
        ...
        trBody += `<tr>
          ...
          row.ProvinceId
													...
													,'${row.ShippingCost}','${row.Discount}','${row.InstallTeam}','${row.Free}','${
          row.Remark
        }','${row.TaxNum}','${row.Phone}'
                                                    )">
                                                    Edit</button>
                                </div>
...
// async function load3(username, userAccountType, show, RG="") {
...
//         let _No = row["No"];
//         let _userId = row["PersonnelNumber"];
//         let _recid = row["RecId"];

//         let _date = row["Date"].toLocaleString();//("th-TH", { timeZone: "UTC" });
//         let _store = row["StoreId"];
//         let _pool = row["Pool"];
//         let _salesId = row["SalesId"];
//         let _purchId = row["PurchId"];
//         let _custName = row["CustName"];
//         let _name = row["EmpName"];
//         let _qty = row["Qty"];
//         let _amount = row["Amount"];
//         let _lineCount = row["LineCount"];
//         let _imgCount = row["ImageCount"];

//         let _region = row["Region"];
//         let _regionName = row["RegionName"];
//         let _province = row["ProvinceId"];
//         let _provinceName = row["ProvinceName"];

//         let _installment = row["Installment"];
//         let _balance = row["Balance"];
//         let _pools = row["CountPools"];

//         let _cancel = row["Cancel"];
//         let _cancelDate = row["CancelDate"].toLocaleString();
//         let _cancelRemark = row["CancelRemark"];
//         let _salesIdAX = row["SalesIdAX"];

//         if  (_provinceName != "")
//           _provinceName =  _provinceName + " / " ;

//         let display = '', displayImg = '', displayCancel = '';
//         if (_cancel == 1) {
//           displayImg = 'block';
//           display = 'none';
//           displayCancel = 'none';
...
//           if (userAccountType > 1)// manager
//             displayCancel = 'block';
//           else
//             displayCancel = 'none';
//         }

//         // if (row["ConfirmDate"] == "1900-01-01T00:00:00" || row["ConfirmDate"] == "1900-01-01 00:00:00.000") {
//         //   _confDate = "-";
//         // }
...
//           <td>
//               <div class="col-sm-12" style="margin: 5px;">
//                 <button type="button" id="btnEdit${_recid}" class="btn btn-block btn-outline-success btn-xs"
//                   style="display: ${display}"
//                   onclick="clickEdit('${_name}','${_store}','${_pool}','${_qty}'
//                                   ,'${_amount}','${_custName}','${_date}'
...
//               </div>
//               <div class="col-sm-12" style="margin: 5px; display: ${displayCancel};">
//                 <button type="button" id="btnCancel${_recid}" class="btn btn-block btn-outline-danger btn-xs"
//                 onclick="clickCancel('${_recid}','${_custName}','${_salesIdAX}')">

//                 Cancel</button>
//               </div>
//               <div class="col-sm-12" style="margin: 5px; display: ${display};">
//               <button type="button" id="btnImage${_recid}" class="btn btn-block btn-outline-info btn-xs" onclick="clickBrowsImg('${_recid}')">

//               Upload image</button>
//               </div>
//               <div class="col-sm-12" style="margin: 5px; display: ${displayImg};">
//               <a style="cursor: pointer; "
//                 onclick="detailCancelDeposit('${_custName}', '${dateFormat(_cancelDate)}', '${_cancelRemark}')">
//                 <img style="with: 48px; height: 48px" src="../images/vecteezy_cancelled-rubber-stamp-on-white-background-vector-illustration_19495927.jpg" alt="Cancel deposit" >
//               </a>
...
//             </div>
//             <div class="col-sm-12">
//               <small> ${_provinceName} ${_regionName}  </small>
//               </div>
//           </td>
...
//               </div>
//               <div class="col-sm-12">
//                 <label style="font-weight: 500;">มัดจำ  </label> ${numberFormat(_installment)} <br>
//                 <label style="font-weight: 500; color: green;">คงเหลือ  </label> ${numberFormat(_balance)}
//               </div>

//           </td>
//           <td style="display: flex; align-items: center;">
//               <div class="col-sm-12">
//                 <label style="font-weight: 500;">${_custName}</label>
//               </div>
//               <div class="col-sm-12">
//                SO : <label style="font-weight: 400;">${_salesId}</label>, PO : <label style="font-weight: 400;">${_purchId}</label>
//             </div>
//           </td>
...
//             <button type="button"  class="btn btn-block btn-outline-info btn-xs"
//               style="display: ${display}"
//               onclick="clickLine('${_recid}','${_pool}','${_name}'
//                                 ,'${_store}','${_qty}'
//                                 ,'${_amount}','${_custName}','${_date}'
//                                 ,'${_salesId}','${_purchId}'
//                                 ,'${_userId}'
//                                 ,'${_region}','${_regionName}','${_provinceName}')">
//                                Line (${_lineCount})
//             </button>
//             <button  type="button" class="btn btn-block btn-outline-info btn-xs"
//             style="display: ${display}"
//             onclick="clickImage('${_recid}')">
//                 View image (${_imgCount})</button>
//             </td>
//         </tr>
//         `;
//         rowcount++;
...
function clickPrint(recId) {
  Swal.fire({
    ...
    html: `
      <input type="radio" id="stm1" name="swal-radio" value="stmm_group">
      <label for="stm1">บริษัท สตาร์มาร์ค กรุ๊ป จำกัด (สำนักงานใหญ่)</label><br>
      <input type="radio" id="stm2" name="swal-radio" value="stmm">
      <label for="stm2">บริษัท สตาร์มาร์ค แมนูแฟคเชอร์ริ่ง จำกัด</label><br>
    `,
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#d33",
    ...
    preConfirm: () => {
      ...
      if (!selectedOption) {
        Swal.showValidationMessage("กรุณาเลือกรายการก่อนกดตกลง");
        return false;
      }
    ...
    if (result.isConfirmed) {
      console.log("คุณเลือก:", result.value);

      window.open(
        `https://localhost:44347/Report/PrintOrderConfirmation/${recId}|${result.value}`,
        // `https://starmark.work/Orderonlinereports/Report/PrintOrderConfirmation/${recId}/${result.value}`,
        "_blank",
