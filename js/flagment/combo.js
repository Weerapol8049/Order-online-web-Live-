...
let SERVER_CB_order = "https://starmark.work/OrderOnline_API_Order_test/";
let SERVER_CB_ax = "https://starmark.work/OrderOnline_API_AIF_test/";
...
function printDeposit(recId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.open(
        `https://localhost:44347/Report/PrintTemporaryReceipt/${recId}|${result.value}`,
        // `https://starmark.work/Orderonlinereports/Report/PrintTemporaryReceipt/${recId}/${result.value}`,
        "_blank",
...
function onchangePayment(value, id) {
  let ret = true;
  let amount = document.getElementById("totalAmount").value; //ยอดสุทธิ
  let total = 0;
